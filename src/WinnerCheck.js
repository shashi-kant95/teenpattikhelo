...
function WinnerCheck() {
    ...
    function getCardValue(card) {
        ...
        return faceCards[card.value] || parseInt(card.value);
    }
    ...
    function isPair(cards) {
        ...
        return new Set(values).size === 2; // If there is a pair, there will be only 2 unique values
    }

    // Function to determine the hand type and value
    function evaluateHand(cards) {
        if (isFlush(cards) && isSequence(cards)) {
            return { type: 'Pure Sequence', value: getCardValue(cards[0]) };
        } else if (isFlush(cards)) {
            return { type: 'Color', value: getCardValue(cards[0]) };
        } else if (isSequence(cards)) {
            return { type: 'Sequence', value: getCardValue(cards[0]) };
        } else if (isPair(cards)) {
            return { type: 'Pair', value: getCardValue(cards[1]) }; // Assuming cards form a pair
        } else {
            return { type: 'High Card', value: getCardValue(cards[2]) }; // Highest card
        }
    }
    ...
    function determineWinner(player1Cards, player2Cards) {
        ...
        if (player1Hand.type !== player2Hand.type) {
            return player1Hand.type.localeCompare(player2Hand.type); // Compare hand types
        } else {
            return player1Hand.value - player2Hand.value; // If hand types are equal, compare hand values
        }
    }

    const getWinner = () => {
        // Determine the winner
        const result = determineWinner(setPlayerACard, setPlayerBCard);
